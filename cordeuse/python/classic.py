...
from PySide6.QtWidgets import QApplication, QFrame, QLabel, QMainWindow, QPushButton, QStackedWidget, QWidget
...
class Led(QLabel):
    ...
    def set_active(self, active: bool):
        ...
        self.setStyleSheet(f"background:{color}; color:white; border:2px inset #aaa;font:bold 12px Arial;")
...
class MachineDrawing(QWidget):
    ...
    def paintEvent(self, event):  # noqa: N802
        ...
        p.drawRoundedRect(45, 225, 390, 80, 12, 12)
        p.setBrush(QColor("#b8bdc1"))
        p.drawRoundedRect(80, 130, 320, 130, 18, 18)
        p.setPen(QPen(QColor("#555"), 10))
        p.drawLine(135, 135, 100, 55)
        p.drawLine(385, 135, 420, 55)
        p.setPen(QPen(QColor("#1d1d1d"), 4))
        p.setBrush(QColor("#d7d7d7"))
        p.drawEllipse(73, 36, 55, 55)
        p.drawEllipse(393, 36, 55, 55)
        p.setBrush(QColor("#333"))
        p.drawEllipse(92, 55, 17, 17)
        p.drawEllipse(412, 55, 17, 17)
        p.setPen(QPen(QColor("#d2c31b"), 3))
        p.drawLine(125, 64, 395, 64)
        p.setBrush(QColor("#23282d"))
        p.setPen(QPen(QColor("#0c0c0c"), 3))
        p.drawRoundedRect(200, 170, 120, 60, 8, 8)
        p.setPen(QColor("#67ff76"))
        p.setFont(QFont("Courier New", 18, QFont.Bold))
        p.drawText(215, 208, f"{self.value:04.1f}")
        p.setPen(QColor("#333"))
        p.setFont(QFont("Arial", 11, QFont.Bold))
        p.drawText(145, 335, "SCHÉMA SIMPLIFIÉ DE LA SP55")
...
class ClassicWindow(QMainWindow):
    def __init__(self):
        ...
        header.setStyleSheet("background:qlineargradient(x1:0,y1:0,x2:0,y2:1,stop:0 #e9edf2,stop:1 #8997a5);border:3px outset #e6e6e6; color:#153863; font:bold 27px Arial;")
        ...
        for y, (text, index) in enumerate((("COMMANDE", 0), ("RÉGLAGES", 1), ("DIAGNOSTIC", 2)), start=0):
            b = self._button(text, nav)
            b.setGeometry(16, 22 + y * 82, 154, 66)
    ...
    def _button(self, text, parent, red=False):
        ...
        button.setStyleSheet(f"QPushButton{{background:{base};color:{color};border:4px outset #eee;font:bold 15px Arial;}}QPushButton:hover{{background:{hover};}}QPushButton:pressed{{border-style:inset;}}")
        return button
    ...
    def _display(self, parent, x, y, w, h, text="00.0"):
        ...
        label.setAlignment(Qt.AlignCenter)
        label.setStyleSheet("background:#07120b;color:#61ff70;border:4px inset #777;font:bold 42px 'Courier New';")
        return label
    ...
    def _control_page(self):
        ...
        minus = self._button("−", target); minus.setGeometry(26, 138, 92, 52); minus.clicked.connect(lambda: self.change_target(-0.5))
        plus = self._button("+", target); plus.setGeometry(142, 138, 92, 52); plus.clicked.connect(lambda: self.change_target(0.5))
        ...
        for y, callback in ((50, self.cycle_pre), (96, self.cycle_knot), (142, self.cycle_speed)):
            b = self._button("MODIFIER", params); b.setGeometry(300, y - 2, 170, 34); b.clicked.connect(callback)

        actions = self._panel(page, 520, 410, 260, 200, "COMMANDE")
        self.start = self._button("DÉMARRER\nLE TIRAGE", actions); self.start.setGeometry(25, 50, 210, 66); self.start.clicked.connect(self.toggle)
        release = self._button("RELÂCHER", actions); release.setGeometry(25, 126, 210, 48); release.clicked.connect(self.release)
        self.counter = QLabel("Cordes tirées : 0", actions); self.counter.setGeometry(25, 174, 210, 22); self.counter.setAlignment(Qt.AlignCenter); self.counter.setStyleSheet("font:bold 13px Arial;color:#20364e;")
        return page

    def _settings_page(self):
        page = QWidget(); page.setStyleSheet("background:#7e8993;")
        title = QLabel("RÉGLAGES GÉNÉRAUX", page); title.setGeometry(10, 12, 770, 48); title.setAlignment(Qt.AlignCenter); title.setStyleSheet("background:#173d69;color:white;border:3px outset #aac;font:bold 22px Arial;")
        y = 90
        for text in ("UNITÉ : kg", "CONTRASTE AFFICHEUR", "CALIBRATION CAPTEUR", "PORT DE COMMUNICATION", "RETOUR PARAMÈTRES USINE"):
            b = self._button(text, page); b.setGeometry(135, y, 520, 72); y += 92
        return page

    def _diagnostic_page(self):
        page = QWidget(); page.setStyleSheet("background:#7e8993;")
        title = QLabel("DIAGNOSTIC ENTRÉES / SORTIES", page); title.setGeometry(10, 12, 770, 48); title.setAlignment(Qt.AlignCenter); title.setStyleSheet("background:#173d69;color:white;border:3px outset #aac;font:bold 22px Arial;")
        y = 92
        for i, name in enumerate(("ARDUINO MEGA", "CAPTEUR D'EFFORT", "FIN DE COURSE AVANT", "FIN DE COURSE ARRIÈRE", "COMMANDE MOTEUR", "ARRÊT D'URGENCE")):
            label = QLabel(name, page); label.setGeometry(120, y, 300, 42); label.setStyleSheet("background:#cbd0d4;border:2px inset #eee;padding-left:15px;font:bold 14px Arial;color:#17385d;")
            led = Led("OK" if i in (1, 2, 3) else "SIMULATION", i in (1, 2, 3), page); led.move(465, y + 7); y += 72
        return page
    ...
    def cycle_pre(self):
        values = [0, 10, 20]; self.state.pre_etirage = values[(values.index(self.state.pre_etirage) + 1) % len(values)]; self.pre_led.setText(f"{self.state.pre_etirage} %")

    def cycle_knot(self):
        values = [0, 10, 20]; self.state.noeud = values[(values.index(self.state.noeud) + 1) % len(values)]; self.knot_led.setText(f"+{self.state.noeud} %")

    def cycle_speed(self):
        self.state.vitesse = self.state.vitesse % 3 + 1; self.speed_led.setText(f"VITESSE {self.state.vitesse}")

    def toggle(self):
        self.state.actif = not self.state.actif; self.state.temps = 0.0; self.start.setText("ARRÊTER\nLE TIRAGE" if self.state.actif else "DÉMARRER\nLE TIRAGE")

    def release(self):
        self.state.actif = False; self.state.mesure = 0.0; self.start.setText("DÉMARRER\nLE TIRAGE")
    ...
    def tick(self):
        if self.state.actif:
            ...
            if self.state.temps > 2.5:
                self.state.actif = False; self.state.cordes += 1; self.start.setText("DÉMARRER\nLE TIRAGE")
        else:
            self.state.mesure *= 0.91
            if self.state.mesure < 0.03: self.state.mesure = 0.0
